...
-- Fixed UUID and field name mismatches
-- Version: 2.0
-- Created: 2025-10-27
-- ============================================
...
-- NOTE: revenue_id is UUID type but SQLAlchemy treats it as string (UUID(as_uuid=False))
CREATE TABLE revenue_records (
    ...
    -- Constraints
    CONSTRAINT revenue_amount_positive CHECK (revenue_amount >= 0)
);

-- Cost Records (from Odoo ERP)
-- NOTE: cost_id is UUID type but SQLAlchemy treats it as string (UUID(as_uuid=False))
CREATE TABLE cost_records (
    ...
    -- Constraints
    CONSTRAINT cost_amount_positive CHECK (cost_amount >= 0)
);

-- Daily Financial Summaries
-- FIXED: Changed from summary_id (UUID) to id (SERIAL INTEGER) to match SQLAlchemy model
-- FIXED: Changed total_cost to total_cogs to match model
-- FIXED: Changed cost_count to cogs_count to match model
-- FIXED: Changed created_at to generated_at to match model
-- FIXED: Removed updated_at (not in model)
...
CREATE INDEX idx_summary_generated_at ON daily_financial_summaries(generated_at DESC);
...
-- NOTE: daily_financial_summaries does not have updated_at column
...
SELECT
    ...
    COALESCE(SUM(r.revenue_amount), 0) - COALESCE(SUM(co.cost_amount), 0) as gross_profit
FROM cells c
...
1. daily_financial_summaries now uses 'id' (SERIAL) instead of 'summary_id' (UUID)
2. Changed 'total_cost' to 'total_cogs' to match SQLAlchemy model
3. Changed 'cost_count' to 'cogs_count' to match model
4. Changed 'created_at' to 'generated_at' to match model
5. Removed 'updated_at' from daily_financial_summaries (not in model)
6. SQLAlchemy uses UUID(as_uuid=False) for revenue_id and cost_id, treating them as strings
...
2. Run this script:
   psql -U nerdx_user -d nerdx_accounting -f init_database_v2.sql
